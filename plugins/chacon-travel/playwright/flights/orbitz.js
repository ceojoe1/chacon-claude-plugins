...
// Orbitz is part of Expedia Group and shares the same uitk-* markup,
// fare-tier modal, and returning-flights layout (see debugging/orbitz.md).
// This scraper is a near-clone of expedia.js with just the base URL swapped.
// Akamai is configured per-brand, so Orbitz often succeeds when Expedia is
// blocked — and vice versa.

// Carriers we drill into. Others (Spirit, Frontier, JetBlue, Alaska) are
// skipped — modal fare-tier names diverge enough per carrier that matching
// cleanly would need per-carrier rules. Keep the working set small.
...
// Simple search URL. Same path/query shape as Expedia (Expedia Group brand);
// avoid the long filter-laden variant from debugging/orbitz.md to dodge
// Akamai's "Access Denied" on fresh sessions.
function buildUrl(params) {
  const fmt = d => { const [y, m, day] = d.split('-'); return `${m}/${day}/${y}`; };
  return (
    'https://www.orbitz.com/Flights-Search?' +
    `trip=roundtrip` +
...
async function search(context, params) {
  ...
  try {
    ...
    await page.goto('https://www.orbitz.com', { waitUntil: 'domcontentloaded', timeout: 20_000 });
    await humanDelay(2000, 3000);
    ...
    console.log(`  [Orbitz] Found ${departingTexts.length} recommended departing flight(s)`);
    departingTexts.forEach((t, i) => {
      console.log(`  [Orbitz] DUMP card[${i}] (${t.length} chars):\n    ${t.slice(0, 350)}\n`);
    });
    ...
    for (let i = 0; i < departingTexts.length; i++) {
      ...
      console.log(`  [Orbitz] --- Card ${i} loop start ---`);
      const carrier = detectCarrier(departingText);
      if (!carrier) {
        console.log(`  [Orbitz] Card ${i}: skipping (carrier not in WN/UA/DL/AA)`);
        continue;
      }
      const carrierConfig = SUPPORTED_CARRIERS[carrier.code];
      const departing = parseCard(departingText);
      if (!departing) {
        console.log(`  [Orbitz] Card ${i}: parseCard returned null for ${carrier.name}`);
        continue;
      }
      console.log(`  [Orbitz] Card ${i}: parsed ${carrier.name} ${departing.timeRange}`);
      ...
      console.log(`  [Orbitz] Card ${i}: clicked ${carrier.name} departing, waiting for modal`);
      const modal = await waitForFareModal(page);
      const modalCount = await modal.count().catch(() => 0);
      console.log(`  [Orbitz] Card ${i}: modal locator count=${modalCount}`);

      const tierPicked = await selectFareTier(page, modal, carrierConfig);
      if (!tierPicked) {
        console.log(`  [Orbitz] Card ${i}: tier selection failed for ${carrier.name}`);
        continue;
      }
      console.log(`  [Orbitz] Card ${i}: tier selected, waiting for returning flights`);

      const returnTexts = await scrapeReturning(page);
      console.log(`  [Orbitz] Card ${i}: got ${returnTexts.length} returning flight card(s)`);
