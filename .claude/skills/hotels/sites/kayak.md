...
### Observed MCP Behavior (2026-04-04)
- Navigate to `kayak.com/stays` — form pre-fills with previous search (destination, dates, guests)
- A sign-in modal may appear on first click; close with the X button (top-right of modal)
- Guests picker opens as a modal with Adults/Children/Rooms and an **Apply** button
- After clicking Apply, click **Search** button — results open in a **new tab**
- Results URL format: `kayak.com/hotels/[city-slug]/[YYYY-MM-DD]/[YYYY-MM-DD]/[N]adults`
- `get_page_text` works on results tab to extract all hotel cards
- Results page shows: property name, city, rating score/10, star class, price per night (including fees, before taxes), and booking source links
